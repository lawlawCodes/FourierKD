...
class AnchorHead(BaseDenseHead):
    ...
    def __init__(
        ...
        if self.use_vfl:
            self.loss_cls = MODELS.build(loss_cls_vfl)
        else:
...
#        _ = self.get_vlf_distillation()
    ...
    def loss_by_feat(
        ...
        self.get_gt_box(bbox_targets_list, featmap_sizes)
        # anchor number of multi levels
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]]
        ...
        _ = self.get_iouaware_distillation(cls_scores, bbox_preds, labels_list, label_weights_list, avg_factor, all_anchor_list)
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox)

    def get_gt_box(self, bbox_single_target: list, feature_size) -> list:
        gt_multilevel_mask_map = []
        for i in range(0, len(bbox_single_target)):
            single_gt_box = bbox_single_target[i]
            H, W = feature_size[i]
            B, N, D = single_gt_box.shape
            single_gt_box = single_gt_box.view(B, N // H // W, -1, D)
            tempindex = torch.count_nonzero(single_gt_box, dim=3)
            index = torch.nonzero(tempindex == 4)
            mask_map = torch.zeros(B, 1, H, W)
            if (index.shape[0]):
                for value in index:
                    batch_item = value[0].item()
                    index_item = value[2].item()
                    mask_map[batch_item, 0, index_item // W, index_item % W] = 1
            gt_multilevel_mask_map.append(mask_map)

        return gt_multilevel_mask_map

    def get_iouaware_distillation(self, cls_score, bbox_pred, labels_list, label_weights_list, avg_factor, all_anchor_list):
        return cls_score, bbox_pred, labels_list, label_weights_list, avg_factor, all_anchor_list
